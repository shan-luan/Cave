...
object CardWidgetsRegistry {
  ...
  private final val IN_ENTRIES: List[InEntry] = new ArrayList[InEntry]()
  private final val OUT_ENTRIES: List[OutEntry] = new ArrayList[OutEntry]()
  ...
  def registerIn(`type`: Class[?], factory: BiFunction[Node.InPort[?], Source[?], Actor]): Unit = {
    IN_ENTRIES.add(new InEntry(`type`, factory))
  }

  /** 注册输出端口的只读显示工厂。目标类型为端口声明类型的父类。 */
  def registerOut(`type`: Class[?], factory: Function[Node.OutPort[?], Actor]): Unit = {
