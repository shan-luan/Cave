...
class AudSegActor(segment: Segment) extends SegActor(segment) {
  ...
  override protected def drawContent(batch: Batch, parentAlpha: Float, visibleStartX: Float, visibleEndX: Float): Unit = boundary[Unit] {
