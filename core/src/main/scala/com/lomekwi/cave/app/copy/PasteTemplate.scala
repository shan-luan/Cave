...
import com.lomekwi.cave.timeline.{Interval, SourceGroup, Timeline, Track}
...
object PasteTemplate {
  case class Entry(track: Track, source: Source[?], range: Interval, origin: Long, group: SourceGroup)
  ...
  def of(sources: util.Collection[Source[?]], timeline: Timeline): PasteTemplate = {
    ...
    for (source <- sources.asScala) {
      ...
      if (track != null) {
        raw.add(Entry(track, source, track.getRange(source), track.getOrigin(source), timeline.getGroup(source)))
      }
    }
  ...
  private def remap(entries: util.List[Entry]): util.List[Entry] = {
    ...
    for (entry <- entries.asScala) {
      ...
      copied.add(Entry(entry.track, dup, entry.range, entry.origin, groupCopy))
    }
    copied
  }
}
