...
abstract class Source[T <: Frame] extends Filter[T] with Serializable {
  ...
  private final val filters: List[Filter[? >: T]] = new FilterList[T](this)

  @transient private var segment: Segment = null
  ...
  final def get(time: Long, track: Track): T = {
    ...
    filters.get(filters.size() - 1).getFilterOut().getData().asInstanceOf[T]
  }
